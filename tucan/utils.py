...
def initialize_model(config):
    ...
    device = (
        "cuda" if config.get("use_gpu", False) and torch.cuda.is_available() else "cpu"
    )
...
def generate_timestamped_filename(
    base_name, extension, use_random_suffix=True, model_name=None
):
    """
    Generates a timestamped filename with optional random suffix and full model name.
    ...
    Args:
        ...
        model_name: Optional model name to include (full name, cleaned for filename use)
    ...
    # Create model name part if model_name is provided
    model_part = ""
    if model_name:
        # Clean the model name for filename use (replace problematic characters)
        clean_model_name = (
        ...
        model_part = f"_{clean_model_name}"
    ...
    if use_random_suffix:
        ...
        filename = f"{base_name}{model_part}_{timestamp}_{random_suffix}.{extension}"
    else:
        filename = f"{base_name}{model_part}_{timestamp}.{extension}"
...
def log_system_prompt_details(
    ...
    Args:
        ...
        user_query_header: Header text for user query
    """
    log_debug("=" * 60, "SYSTEM")
    log_debug("SYSTEM PROMPT CONFIGURATION", "SYSTEM")
    log_debug("=" * 60, "SYSTEM")
    log_debug(f"System Prompt Content:\n{system_prompt_content}", "SYSTEM")
    log_debug(f"Functions Header: {functions_header}", "SYSTEM")
    log_debug(f"User Query Header: {user_query_header}", "SYSTEM")
    log_debug("=" * 60, "SYSTEM")
